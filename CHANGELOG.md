...
| Version | Notes |
| ------- | ----- |
| [**16.0.0** (unreleased)](https://github.com/rtfeldman/elm-css/tree/16.0.0) | Switch to using phantom types for `Value`. Overhaul `boxShadow` API. Remove `fontFeatureSettings` and `fontFeatureSettingsList` as the MDN docs says `font-variant` properties should be preferred to `font-feature-settings`. Remove `fontVariant` shorthand; it's too error-prone to be worth it. (Use specific longhand properties like `fontVariantCaps` instead.) Add `times` and `dividedBy`. (#441)
| [**15.0.0** (unreleased)](https://github.com/rtfeldman/elm-css/tree/15.0.0) | Remove `asPairs`, `Css.Namespace`, and arithmetic operators. Don't report warnings, just emit CSS. Make `Property` opaque and more efficient. Fix `withMedia` nesting bug. (#352) Fix bug in `borderBottomWidth` functions. (#380) Add more `lazy` functions and make them accept Styled Html. (#440) Rename `Css.Foreign` to `Css.Global`. (#360) Remove `Css.Colors`. (#358) Remove experimental border properties. (#438) Remove experimental `dir` pseudo-class. (#442)
