...
| Version | Notes |
| ------- | ----- |
| [**14.0.0**](https://github.com/rtfeldman/elm-css/tree/14.0.0) | Remove `asPairs`, `Css.Namespace`, and arithmetic operators. Don't report warnings, just emit CSS. Make `Property` and `Value` opaque, more efficient, and more self-documenting. Use e.g. `float left_` and `float right_` instead of reusing the `left` and `right` functions from `left (px 10)`. Fix `withMedia` nesting bug. (#352)
