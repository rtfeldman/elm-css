...
| Version | Notes |
| ------- | ----- |
| [**16.0.0** (unreleased)](https://github.com/rtfeldman/elm-css/tree/15.0.0) | Remove `asPairs`, `Css.Namespace`, and arithmetic operators. Don't report warnings, just emit CSS. Make `Property` opaque and more efficient. Rename `Css.Foreign` to `Css.Global`. (#360) Fix `withMedia` nesting bug. (#352)
| [**15.0.0**](https://github.com/rtfeldman/elm-css/tree/15.0.0) | Add first-class support for animation keyframes. (#431) Remove experimental border properties. (#438) Remove experimental `dir` pseudo-class. (#442) Remove `Css.Colors`. (#358)
