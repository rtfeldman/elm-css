...
your stylesheet.
    ...
    stylesheet { name = "homepage" }
        $ body
            ...
            ~ color (rgb 7 7 7)
-}
stylesheet : { a | name : name } -> Style class id name
stylesheet { name } =
...
{-| A [type selector](https://developer.mozilla.org/en-US/docs/Web/CSS/Type_selectors).
    ...
    stylesheet "homepage"
        $ body
            ...
            ~ color (rgb 7 7 7)
-}
($) : Style class id name -> Tag -> Style class id name
($) style tag =
    let
        selector =
...
-}
(#) : Style class id name -> id -> Style class id name
(#) style id =
    let
        selector =
...
{-| A [class selector](https://developer.mozilla.org/en-US/docs/Web/CSS/Class_selectors).
    ...
    stylesheet "login"
        button . LoginFormButton
            ...
            ~ color (rgb 128 64 32)
-}
(.) : Style class id name -> class -> Style class id name
(.) style class =
    case style of
        NamespacedStyle name declarations ->
            declarations
    ...
    stylesheet "homepage"
        ...
        @ media print
            $ body
                ~ width (pct 100)
-}
(@) : Style class id name -> String -> Style class id name
(@) style rule =
    case style of
        NamespacedStyle name declarations ->
            declarations
...
...has the same result as:
    ...
    a . FancyLink
        ...
        &: hover
            ~ textDecoration underline
-}
(~=) : Style class id name -> Style class id name -> Style class id name
(~=) style mixinToApply =
...
-}
(~) : Style class id name -> ( String, String ) -> Style class id name
...
{-| -}
(&:) : Style class id name -> PseudoClass -> Style class id name
(&:) style pseudoClass =
    extendTypeSelector
        "&:"
...
{-| -}
(&::) : Style class id name -> PseudoElement -> Style class id name
(&::) style pseudoElement =
    extendTypeSelector
        "&::"
...
extendTypeSelector : String -> (name -> CompoundSelector -> CompoundSelector) -> Style class id name -> Style class id name
